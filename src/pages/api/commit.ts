...
export const POST: APIRoute = async ({ request, cookies }) => {
    ...
    try {
        ...
        delete (body as any).gitset_key;

        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 120000);
        const response = await fetch(`${process.env.CORE_API_URL}/api/commit`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        console.error('commit proxy error:', error instanceof Error ? error.message : error);
